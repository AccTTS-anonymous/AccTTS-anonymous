#!/usr/bin/env bash
# Drives the Llama-3.2-1B-Instruct sweep described in LLAMA1B_SWEEP.md.
#
# Order (Steps 3-6):
...
#
# Per-step logs land in logs/llama1b/. Master status is printed to stdout
# (caller is expected to redirect to a log).
#
# HF_TOKEN must be exported in the launch env (Llama-3.2 is gated). The driver
# refuses to start without it so we don't waste a vLLM init only to 401 on
# weight download.
...
: "${HF_TOKEN:?HF_TOKEN must be exported before invoking this driver (Llama-3.2 is gated)}"
...
set_recipe_beam() {
  # $1=n  $2=beam_width  $3=gemm_opt  $4=chunk_size  $5=dataset_state
...
echo "[$(ts)] LLAMA1B_SWEEP_COMPLETE"
